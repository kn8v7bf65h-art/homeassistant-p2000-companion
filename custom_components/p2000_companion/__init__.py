...
async def _async_register_frontend(hass: HomeAssistant) -> None:
    ...
    if not WWW_PATH.is_dir():
        _LOGGER.error(
            "P2000 Companion www directory not found: %s",
            WWW_PATH,
        )
        return
    ...
    if not card_path.is_file():
        _LOGGER.error(
            "P2000 Companion Lovelace card not found: %s",
            card_path,
        )
        return

    # Mark registration before awaiting, so concurrently loading config entries
    # cannot try to register the same HTTP route a second time.
    ...
    try:
        ...
        # This can occur during development when an earlier version already
        # registered the same route during the current Home Assistant run.
        _LOGGER.warning(
            "P2000 Companion frontend route was already registered: %s",
            err,
        )
    ...
    try:
        ...
        _LOGGER.warning(
            "Could not automatically load the P2000 Companion card. "
...
async def async_migrate_entry(
    ...
    if entry.version == 1:
        monitor_name = str(
            combined.get(CONF_MONITOR_NAME)
            or entry.title
            or DEFAULT_NAME
        ).strip()

        feed_url = str(
            combined.get(CONF_FEED_URL)
            or DEFAULT_FEED_URL
        ).strip()
        ...
        migrated_data = {
            ...
            CONF_SERVICES: [
                ...
                for item in _list_value(CONF_SERVICES)
            ],
            CONF_PRIORITIES: [
                item.upper().replace(" ", "")
            ...
            CONF_TEXT_CONTAINS: [
                item.lower()
                for item in _list_value(CONF_TEXT_CONTAINS)
            ],
            CONF_EXCLUDE_WORDS: [
                item.lower()
                for item in _list_value(CONF_EXCLUDE_WORDS)
            ],
...
async def async_setup_entry(
    ...
    # Do not register the frontend here.
    # It is registered once globally from async_setup().
    ...
    if provider == PROVIDER_TELEGRAM:
        coordinator = P2000TelegramCoordinator(
            hass,
            entry,
        )
