...
async def _async_register_frontend(hass: HomeAssistant) -> None:
    """Register and load the bundled Lovelace frontend once."""
    ...
    try:
        ...
        _LOGGER.warning("P2000 Companion frontend route was already registered: %s", err)
...
async def async_setup(hass: HomeAssistant, config: dict[str, Any]) -> bool:
    """Set up shared P2000 Companion resources."""
...
async def async_migrate_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if entry.version == 1:
        monitor_name = str(combined.get(CONF_MONITOR_NAME) or entry.title or DEFAULT_NAME).strip()
        ...
        migrated_data = {
            ...
            CONF_SERVICES: [normalize_service(item) for item in _list_value(CONF_SERVICES)],
            CONF_PRIORITIES: [item.upper().replace(" ", "") for item in _list_value(CONF_PRIORITIES)],
            CONF_TEXT_CONTAINS: [item.lower() for item in _list_value(CONF_TEXT_CONTAINS)],
            CONF_EXCLUDE_WORDS: [item.lower() for item in _list_value(CONF_EXCLUDE_WORDS)],
            CONF_SCAN_INTERVAL: int(combined.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL)),
            CONF_CREATE_FEED_SENSOR: bool(combined.get(CONF_CREATE_FEED_SENSOR, DEFAULT_CREATE_FEED_SENSOR)),
        }
        hass.config_entries.async_update_entry(
            entry, title=monitor_name, data=migrated_data, options={}, version=5
        )
        return True
...
async def _async_remove_unused_feed_sensor(hass: HomeAssistant, entry: ConfigEntry) -> None:
    """Remove the optional feed sensor when disabled."""
    ...
    entity_id = registry.async_get_entity_id("sensor", DOMAIN, f"{entry.entry_id}_feed")
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if provider == PROVIDER_TELEGRAM:
        ...
        coordinator = P2000ApiCoordinator(hass, entry)
        await coordinator.async_load_cache()
        await coordinator.async_config_entry_first_refresh()
    else:
